...
##        print 'SETUP', self.id1, p1, self.id2, p2
##        print 'SETUP', log.active_games

    def tearDown(self):
        log.active_games = {}

    def testbar_roll_in_1a(self):
        sequence = (
                ((3, 6), []),
                ((6, 3), []),
                ((1, 6), ['bar-1',]),
        ...
        self.spiel.dice_and_moves(sequence, self.id1)
    ...
    def testbar_roll_in_1b(self):
        ...
        game, player = log.get(turn)
        gc = game.control
        gc.position = [-1, 2,0,2,2,2,3, 0,0,0,0,1,0,
                        1,1,-1,0,-2,-3, -3,1,0,-2,-2,0, 0]
        gc.set_position()
        gc.bar = {'p1':0, 'p2':1}
        gc.set_move()
        for d in range(len(sequence)):
